...
LOCATION_COORDS = {
    "London (default)": (51.5074, -0.1278),  # Model training location
    "Edinburgh":        (57.0,   -3.2),       # Scottish Highlands wind
    "Cardiff":          (51.5,   -3.2),       # Wales
    "Aberystwyth":      (52.41,  -4.08),      # Mid Wales wind farms
    "Manchester":       (53.5,   -2.2),       # Northern England
    "Birmingham":       (52.5,   -1.9),       # Midlands
    "Aberdeen":         (57.1,   -2.1),       # North Sea wind
    "Exeter":           (50.7,   -3.5),       # SW England solar
    "Penzance":         (50.1,   -5.5),       # Highest sunshine hours in UK
}
...
def make_dummy_forecast(start_dt, n_periods=336):
    ...
    times = pd.date_range(start=start_dt, periods=n_periods, freq="30min", tz="UTC")
    np.random.seed(42)
...
def make_carbon_line(df: pd.DataFrame, hypothetical=None) -> go.Figure:
    ...
    fig.update_layout(
        **PLOTLY_LAYOUT,
        title=dict(text="Carbon Intensity — Predicted vs Modified (gCO₂/kWh)",
                   font=dict(size=12, color="#e6edf3")),
        height=300,
    )
    fig.update_layout(yaxis=dict(gridcolor="#21262d", linecolor="#21262d",
                                 zeroline=False, title="gCO₂/kWh"))
    return fig
...
def make_storage_chart(df: pd.DataFrame, storage_series: pd.Series) -> go.Figure:
    ...
    fig.update_layout(
        **PLOTLY_LAYOUT,
        title=dict(text="Storage Required / Surplus per 30-min Period (MWh)",
                   font=dict(size=12, color="#e6edf3")),
        height=260,
    )
    fig.update_layout(yaxis=dict(gridcolor="#21262d", linecolor="#21262d",
                                 zeroline=False, title="MWh"))
    return fig
...
def make_mix_pie(mix_mw: dict) -> go.Figure:
    total = sum(mix_mw.values()) or 1
    labels = [SOURCE_LABELS[s] for s in SOURCES if mix_mw.get(s, 0) / total * 100 > 1]
    values = [mix_mw[s]        for s in SOURCES if mix_mw.get(s, 0) / total * 100 > 1]
    colors = [SOURCE_COLORS[s] for s in SOURCES if mix_mw.get(s, 0) / total * 100 > 1]
...
with st.sidebar:
    ...
    app_mode = st.radio("App Mode", ["Historical", "Forecast"], horizontal=True,
                        label_visibility="collapsed")

    # ── Date Range ──
    st.markdown('<div class="section-header" style="margin-top:20px;">Date Range</div>', unsafe_allow_html=True)
    date_mode = st.radio("Date Mode", ["Single Day", "Date Range"], horizontal=True,
                         label_visibility="collapsed")
    ...
    location = st.selectbox("GB Region",
        ["London (default)", "Edinburgh", "Cardiff", "Aberystwyth",
         "Manchester", "Birmingham", "Aberdeen", "Exeter", "Penzance"],
        label_visibility="collapsed")
...
<div style='display:flex;align-items:center;justify-content:space-between;
    ...
    <div>
        ...
        <div style='font-size:13px;color:#8b949e;margin-top:4px;'>
            {location} · {date_range_str} · {len(df):,} half-hourly periods · {mode_label}
        </div>
    </div>
...
with st.expander("⚙  Configure Hypothetical Mix", expanded=True):
    ...
    for src in SOURCES:
        if src in df.columns and hypo_total > 0:
            proportion = hypo_mix[src] / hypo_total
            hypo_series += df["total_supply_mw"] * proportion

    # Positive = surplus, Negative = storage needed (MWh per 30-min period)
    storage_series = (hypo_series - df["total_supply_mw"]) * 0.5

    total_storage_needed_gwh = abs(storage_series[storage_series < 0].sum()) / 1000
    total_surplus_gwh        = storage_series[storage_series > 0].sum() / 1000
    net_gwh                  = storage_series.sum() / 1000
    sf_color = "#3fb950" if net_gwh >= 0 else "#f85149"
    sf_label = "Net Surplus" if net_gwh >= 0 else "Net Storage Needed"
    ...
    with rc4:
        ...
        <div class="metric-card">
            ...
            <div class="metric-value" style="color:{sf_color};">{abs(net_gwh):.1f}</div>
            <div class="metric-unit">GWh across period</div>
        </div>""", unsafe_allow_html=True)
...
with mod_left:
    ...
    st.plotly_chart(make_modified_mix_bar(df, hypo_mix), use_container_width=True,
...
# ─────────────────────────────────────────────
# SECTION 4 — CARBON INTENSITY: PREDICTED vs MODIFIED
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# SECTION 5 — SUPPLY VS HYPOTHETICAL
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# SECTION 6 — STORAGE PER PERIOD
# ─────────────────────────────────────────────
st.markdown('<div class="section-header">Storage Required / Surplus per Period</div>', unsafe_allow_html=True)
st.markdown(f"""
<div style='font-size:11px;color:#8b949e;margin-bottom:12px;'>
    🟢 Green = surplus energy available &nbsp;|&nbsp; 🔴 Red = storage needed &nbsp;|&nbsp;
    Total storage needed: <span style='color:#f85149;'>{total_storage_needed_gwh:.1f} GWh</span> &nbsp;|&nbsp;
    Total surplus: <span style='color:#3fb950;'>{total_surplus_gwh:.1f} GWh</span>
</div>
""", unsafe_allow_html=True)
st.plotly_chart(
    make_storage_chart(df, storage_series),
    use_container_width=True, config={"displayModeBar": False}
)
...
with st.expander("📋  Raw Data"):
    ...
    rename_map.update({"carbon_intensity": "Carbon Intensity (gCO₂/kWh)",
                        "total_supply_mw": "Total Supply (MW)"})
    display_df = display_df.rename(columns=rename_map)
    show_cols = (["datetime"]
                 + [SOURCE_LABELS[s] for s in SOURCES if SOURCE_LABELS[s] in display_df.columns]
...
<div style='border-top:1px solid #21262d;margin-top:32px;padding-top:16px;
    ...
    <span>GRID-ish-Zero · Historical &amp; Forecast mode</span>
